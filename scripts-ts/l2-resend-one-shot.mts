...
const parsed = JSON.parse(canonical) as {
  ok?: unknown;
  findings?: ReadonlyArray<{ claim?: { id?: unknown }; severity?: unknown; plainLine?: unknown }>;
};
...
const verdictLine = parsed.ok
  ...
  : `FAIL — violations present (${findings.length} finding(s))`;

const subject = `[SIMULATED] Curbside Commons audit — statement 2026-06 (simulated)`;
const bodyText = [
  SIMULATED_BANNER,
  ...
  ...findings
    .slice(0, 20)
    .map((f) => `- [${String(f.severity ?? "unknown")}] ${String(f.plainLine ?? "")} (${String(f.claim?.id ?? "")})`),
  ...(findings.length > 20 ? [`...and ${findings.length - 20} more finding(s) — full report attached (report.json).`] : []),
  "",
...
| Payload sha256 | \`${payloadSha256}\` (subject+body+attachment; reproducible offline: same engine, same fixture) |
