...
| 6 | `sample-6-instrument.html` | **Instrument** | *(added 2026-07-08, eighteenth session — a DIFFERENT genre from 1–5)* World-displayable STORY surface: light-ground evidence instrument (gallery-white `#FFFFFF` ground **per the owner word "no dark background" — the first dark-first cut was revised in place, content byte-unchanged**; deep green-tinted ink `#17221E`, faint 24px blueprint grid, ONE deep-viridian accent `#0A6349`, all text ≥5.7:1), full narrative arc (problem → instrument → real evidence → eval discipline → honest close) with 7 real findings + receipts. Copy = `sample-6-copy-deck.md` (domain-expert writing agent, every claim repo-grounded, anti-slop audited; orchestrator filled the live test count 947+6). Does NOT reopen the Ledger pick — `/report`+`/demo` are implemented Ledger; #6 is a candidate for the world-facing landing/story page. |
