...
- **Free-first runtime stack (owner-set, 2026-06-11):** every tool/service in the **product runtime** defaults to **free, free-tier, or self-hostable OSS** — no paid subscriptions, no metered overages. The **only paid runtime exception is the Gemini API** (best current model per the bullet above; **$5 hard total budget**; spend tracked). Free-tier limits are verified against current official pricing pages at selection time (`RULES.md` §6); any tool whose free tier can't carry the slice routes back to the owner before adoption. Dev tooling (Claude/Codex) is unaffected and stays within its existing plans.
