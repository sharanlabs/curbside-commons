...
- **Remaining work — DEPLOY ONLY (T13), all OWNER-GATED.** A **pre-deploy devil's-advocate grill** (Codex cross-model + advisor) was run: DEPLOY-READINESS was NOT-YET → **all honesty copy/disclaimers are now hardened** (non-affiliation footer on every surface · Metrics de-"Impact"-ified · "proven"→"exercised" · static-fixture label + local-verify command · "reference prototype" framing). **3 OWNER-DECISIONS now gate the public deploy:**
  1. **[#1 embarrassment risk]** real **personal-name DBAs** (e.g. "Aaron Jevon Burrus") render as fabricated "high-risk / bank-blocked" under DoorDash framing. **Recommend: swap the PUBLIC demo to obviously-fictional names** (keep the DataSF real-name-provenance story in the docs — it doesn't weaken the "real public-record names" capability claim). I can implement this on your GO (regenerate the snapshot with fictional names, ~1 step). Owner chose keep+label earlier for the *local* build; this is the *public* call.
  2. **"Curbside Commons"** trademark/name clearance (quick web search found no obvious collision — not legal clearance) — or switch to "Demo Marketplace".
  3. Exclude **`out/`** (legacy "DoorDash" strings) from any public **GitHub** push (`.vercelignore` already covers Vercel; the deploy itself is safe).
  Then: Vercel deploy (REPLAY-only; live AI gated off; confirm `.env`/`ENABLE_LIVE_AI` absent in the Vercel env). **No non-owner-gated build work remains** — do not manufacture low-value work.
