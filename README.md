...
- The web console is desktop-only. The identity separation is **implemented in this repo** (the legacy activation surfaces live under `/legacy/**` with their own provenance; the canonical pages present the truth-audit engine) — and the site is **deployed and verified live on Vercel** at `curbside-commons.vercel.app` (2026-07-27: all six canonical routes 200, all four security headers confirmed on a subpath). An older Cloudflare Pages deployment at `curbside-commons.pages.dev` is **stale and pending retirement** — treat the Vercel URL as canonical.
