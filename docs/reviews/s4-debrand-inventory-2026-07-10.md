...
**Method:** full tracked-file grep (`git grep -E`) over `DoorDash | Door Dash | Uber Eats | Uber&nbsp;Eats | UberEats | Grubhub | GrubHub | \bSquare\b | OpenAI | \bStripe\b | \bGoogle\b | DataSF | Instacart | \bToast\b` — the five-name set plus every F-16 expansion (OpenAI/Stripe/Google/DataSF), the `&nbsp;` encoding, spelling variants, and two adjacent marketplace brands (Instacart, Toast) found during the sweep. Unicode-NBSP variant (`Uber Eats`): **zero hits** repo-wide. 163 files carry ≥1 hit; every one is classified below. `ActivationOps` self-name hits are **out of S4 scope by directive** (the de-brand order targets external marketplace/company brands; the repo's own former name is the S5 identity lane — see §7).
...
- Dated plans/specs/status docs: `docs/plan-*.md` (5 incl. this workstream's own plan), `docs/phase3-prep-slice-plan.md`, `docs/spec-semantic-judge.md`, `docs/story-arc-content-spec.md` (cites the 2026-06-23 incumbent design research — Gate-P2 citation class), `docs/architecture/agentic-architecture-blueprint.md`, `docs/t002-slice-plan.md`, `docs/v1-slice-plan.md`, `docs/v1-data-dictionary.md` + `docs/data-audit.md` (both record the original CSV filename as a dated audit fact; the CSV itself was renamed long ago per OQ-1), `docs/strategist-confirmatory-status.md`, `docs/a3-7-live-run-status.md`, `docs/tooling-and-skills-usage.md`.
