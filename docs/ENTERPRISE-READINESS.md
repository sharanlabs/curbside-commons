...
- **Live-LLM assurance:** the eval was **exercised** over the *real* model output in a recorded live run (`lib/data/live-samples.snapshot.json`, $0.0042) — it caught no bad-but-parseable draft that run, though the run surfaced + fixed a real guardrail-precision bug. A **calibrated LLM-judge** for *semantic* unsupported-claim detection remains not-yet-built (the deterministic graders + claims-gatekeeper are forward-only — a documented boundary).
