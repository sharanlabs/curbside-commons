...
- **Eval harness** (`lib/evals/draft-quality.ts`) — deterministic graders (structure · state-consistency · policy · no-leakage), each paired with a corrupted record it must catch (teeth, not theater); the no-leakage rule is shared with the runtime gatekeeper.
...
- **Quality gate** — `npm run verify` (typecheck · lint · test · build); 159 tests + 3 Playwright e2e including the differential + the guardrail-corpus + draft-text differentials; `next build` prerenders every route (also the console render-smoke).
