# Archived: docs/implementation-journal.md — entries closed before 2026-07-08

**Archived 2026-07-31 (session 40).** Byte-exact. Nothing edited, summarised, or
deleted. Split by the date on each entry, not by line position — both files were
appended out of chronological order, so a positional cut would have mis-sorted them.

Companion to the state-doc archive in this directory; same reason
(`docs/assessment-2026-07-30-estate-revaluation.md` finding 2 — accretion in the
docs every session must read). `decision-log` and `implementation-journal` are not
in the `AGENTS.md`:7 startup list, so they were never part of the gate failures —
they were on the same trajectory.

Read for provenance. The live file carries anything still operative.
