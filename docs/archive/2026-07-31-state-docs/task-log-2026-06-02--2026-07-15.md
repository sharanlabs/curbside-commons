# Archived: docs/task-log.md — sessions closed on or before 2026-07-30

**Archived 2026-07-31 (session 40).** This is the byte-exact historical narrative
moved out of `docs/task-log.md` so the startup-contract read stays affordable. Nothing was
edited, summarised, or deleted — the lines below are the original text, in the
original order.

**Why:** `docs/assessment-2026-07-30-estate-revaluation.md` finding 2. The
startup contract (`RULES.md` §15, `AGENTS.md`:7) orders every agent — Codex
reviewers included — to read these docs first. They had reached 1.31 MB, and
three cross-model gate runs died on it (sessions 34, 36, 38). The instruction was
the trigger; the volume was the cause, so the volume is what moved.

**Still authoritative for history.** Read this when you need provenance for a
decision. Do not read it as current state — the live doc carries that.
