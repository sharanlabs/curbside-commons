...
### Make it SHOWABLE (the demo surface — REPLAY-safe, $0) — owner ask 2026-06-22
The judge must be a **visible, demonstrable feature**, not just a backend control + a metrics file. Three surfaces, all renderable in the free public demo from a **recorded judge fixture** (frozen during the P3 calibration run, exactly like `lib/data/live-samples.snapshot.json`) so the public REPLAY demo shows REAL per-claim verdicts at **zero spend**:
1. **Merchant Detail → a "Faithfulness check" panel** (a new step in the why-chain, right after the gatekeeper): renders the judge's **per-claim verdicts on that merchant's draft** — each prose sentence ✓ supported (→ the field that backs it) / ✗ unsupported. This is the centerpiece: a viewer literally watches each line of the AI's message get checked against the data.
2. **Eval page → a calibration dashboard**: precision / recall / F1, the confusion matrix, Cohen's κ, and the test-retest flip-rate from the calibration run — so **"calibrated" is visible and credible**, not just asserted.
3. **A "catch in action" exhibit**: one draft containing a real/planted fabrication shown being **flagged** by the judge (sourced from the gold set / the live snapshot) — the money shot for a marketplace demo ("here's the AI making something up; here's the control catching it").

Design rule: live judge runs only behind the key (P3); the public demo renders the recorded fixture (REPLAY, $0) — same honesty posture as today (labeled "recorded run").
...
- **P1 — judge + mock path + the visible panel (offline, no spend):** `lib/agents/semantic-judge.ts` (Zod schema, prompt, mock + live(gated) like `draft.ts`); wire as the SECONDARY control after the gatekeeper; budget-ledgered; unit tests on the mock path. **Build the Merchant-Detail "Faithfulness check" panel now** (rendering mock per-claim verdicts) so the feature is showable from day one. `lib/core` untouched.
