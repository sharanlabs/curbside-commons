...
**Why hybrid data (real-data capability + synthetic activation), with a fictional public display.** The adapter ingests real SF businesses (DataSF, public domain) to prove the real-data path; synthetic onboarding state exists because no public dataset does. *Rejected:* fully synthetic (invites "you designed the inputs"), or scraping a real dataset (ToS/PII/redistribution risk). **But the public demo DISPLAYS fictional names, not the real ones** — attaching synthetic "high-risk / stalled" states to real businesses or sole-proprietor personal-name DBAs on a public page is a privacy/optics harm (the pre-deploy grill's #1 finding). So: real-data *capability* (the adapter + its tests, license-clean), fictional *display*. (NAICS is sector-level, so the adapter's real layer is Restaurant/Retail only — Grocery/Convenience are left to synthetic, not fabricated.)
...
**Honest production gaps (the adoption boundary).** Adoption-*grade* means the architecture, controls, evals, the real-data adapter, and a documented adoption path are credible enough to inherit — **not** "production-ready." Known gaps: auth/multi-tenancy, real integrations (Slack/email/CRM), persistence/observability at scale, a **calibrated LLM-judge** for semantic unsupported-claims (the deterministic eval — incl. the no-leakage grader — caught register/disclosure leaks in 3 of the 6 parseable real drafts, and the gatekeeper now blocks them; a calibrated *semantic* judge remains unbuilt), and the deeper blocker root-causes that need instrumentation (named in `lib/domain/diagnosis.ts`, not faked). A marketplace inherits it by swapping the real layer of the hybrid dataset for its own export against the adapter's documented contract.
