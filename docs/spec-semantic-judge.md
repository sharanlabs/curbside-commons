...
- **Groq free tier** (no card): `openai/gpt-oss-120b` / `gpt-oss-20b` support **strict JSON-schema** (constrained decoding, 100% adherence); free-tier `gpt-oss-120b` ≈ 30 RPM / 1K req-day / 200K tok-day — ample for a paced ~100-item × 3–5-rep calibration. (console.groq.com/docs/structured-outputs, /docs/rate-limits.)
