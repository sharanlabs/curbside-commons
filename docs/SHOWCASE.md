...
| Classifier tool | advisory candidates, `earnsLabel: false` (the injected sync classifiers never earn); **the LIVE lane: "calibrated (fresh held-out, pre-registered floors, one pass)" — the owner-armed 2026-07-09 retry cleared all six floors 21/21 on a fresh pre-registered split** (the 2026-07-05 near-miss DEFER stands on record; the split's authorship-provenance caveat + its mechanical mitigation are recorded in `docs/fee-classifier-recalibration-status.md`) | any real-world-statement performance claim (the exam is a synthetic n=21 gold set); any label for the injected baseline/mock |
